...
export default function EventDetailPage() {
  ...
  const fetchEvent = async () => {
    try {
      ...
      if (isAuthenticated && user && response.data.data.registration?.registeredParticipants) {
        const registered = response.data.data.registration.registeredParticipants.some(
          p => p.user?._id === user._id || p.user?.toString() === user._id
        );
