...
export default function OurPartnersTable() {
  const { partners, isLoading, getPartners, deletePartner, updatePartner } = useAdminStore();
  ...
  const hospitalPartners = partners
    .filter(p => p.businessType === 'hospital' && (p.status === 'approved' || p.isActive))
    .filter(p => 
      p.name?.toLowerCase().includes(hospitalSearchTerm.toLowerCase()) ||
      p.contactPerson?.name?.toLowerCase().includes(hospitalSearchTerm.toLowerCase()) ||
      p.email?.toLowerCase().includes(hospitalSearchTerm.toLowerCase()) ||
      p.phone?.includes(hospitalSearchTerm) ||
      p.address?.city?.toLowerCase().includes(hospitalSearchTerm.toLowerCase())
    );

  // Doctor Partners (medical category but not hospital)
  const doctorPartners = partners
    .filter(p => p.category === 'medical' && p.businessType !== 'hospital' && (p.status === 'approved' || p.isActive))
    .filter(p => 
      p.name?.toLowerCase().includes(doctorSearchTerm.toLowerCase()) ||
      p.contactPerson?.name?.toLowerCase().includes(doctorSearchTerm.toLowerCase()) ||
  ...
  const foodPartners = partners
    .filter(p => p.category === 'food' && (p.status === 'approved' || p.isActive))
    .filter(p => 
      p.name?.toLowerCase().includes(foodSearchTerm.toLowerCase()) ||
      p.contactPerson?.name?.toLowerCase().includes(foodSearchTerm.toLowerCase()) ||
  ...
  return (
    <>
      <div className="mb-8 bg-white rounded-xl shadow-lg p-4 border-2 border-gray-100">
        <h1 className="text-2xl font-bold bg-gradient-to-r from-green-600 to-blue-600 bg-clip-text text-transparent mb-2">Partner's</h1>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl border-2 border-gray-200 p-6 backdrop-blur-sm">
            ...
            <div className="overflow-x-auto">
              <div className="max-h-[400px] overflow-y-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: '#9CA3AF #F3F4F6' }}>
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-gradient-to-r from-red-500 via-pink-500 to-red-500">
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Hospital Name</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">License No.</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Specializations</th>
                      ...
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Bed Capacity</th>
                  ...
                  {hospitalPartners.length > 0 ? (
                    hospitalPartners.map((partner, idx) => {
                      ...
                      if (metadata.specializations && Array.isArray(metadata.specializations)) {
                        specializations = metadata.specializations;
                      } else if (metadata.specializations && typeof metadata.specializations === 'string') {
                        specializations = metadata.specializations.split(',').map(s => s.trim()).filter(s => s);
                      } else if (partner.description) {
                        const match = partner.description.match(/Specialization[s]?:\s*([^.]+)/i);
                        if (match) {
                          specializations = match[1].split(',').map(s => s.trim()).filter(s => s);
                        }
                      }
                      ...
                      if (metadata && metadata.bedCapacity) {
                        ...
                        if (bc && bc !== '' && bc !== '0' && bc !== 'null' && bc !== 'undefined') {
                          bedCapacity = bc;
                        }
                      }
                      ...
                      if (!bedCapacity && partner.adminNotes) {
                        const match = partner.adminNotes.match(/bed capacity[:\s]*(\d+)/i);
                        if (match && match[1]) {
                          bedCapacity = match[1].trim();
                        }
                      }
                      ...
                      const contactPersonName = partner.contactPerson?.name && partner.contactPerson.name !== partner.name 
                        ? partner.contactPerson.name 
                        : null;
                      
                      return (
                        <tr key={partner._id} className={`border-b border-gray-200 hover:bg-gradient-to-r hover:from-red-50 hover:to-pink-50 transition-all duration-200 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                          <td className="px-4 py-4">
                            <div className="font-medium text-gray-900">{partner.name}</div>
                            {contactPersonName && (
                              <div className="text-xs text-gray-500">Contact: {contactPersonName}</div>
                            )}
                          </td>
                          <td className="px-4 py-4 text-sm text-gray-700">
                            {partner.documents?.businessLicense || partner.documents?.gstNumber || 'N/A'}
                          </td>
                          <td className="px-4 py-4">
                            {specializations.length > 0 ? (
                              <>
                                ...
                                {specializations.length > 2 && (
                                  <div className="text-xs text-gray-500">+{specializations.length - 2} more</div>
                                )}
                              </>
                            ) : (
                          ...
                          <td className="px-4 py-4 text-sm">
                            <div className="text-gray-700">{partner.email || partner.contactPerson?.email || 'N/A'}</div>
                            <div className="text-gray-600">{partner.phone || partner.contactPerson?.phone || 'N/A'}</div>
                          </td>
                          <td className="px-4 py-4 text-sm text-gray-700 max-w-xs">
                            <div className="line-clamp-2">
                              {partner.address?.city || 'N/A'}, {partner.address?.state || 'N/A'}
                            </div>
                          </td>
                          <td className="px-4 py-4 text-sm text-gray-700">
                            {bedCapacity && bedCapacity !== '0' && bedCapacity !== '' ? `${bedCapacity} beds` : 'N/A'}
                          </td>
                          <td className="px-4 py-4">
                            <div className="flex gap-2">
                              <button 
                                onClick={() => handleEdit(partner)}
                                className="px-3 py-1.5 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-xs rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all shadow-md hover:shadow-lg flex items-center gap-1"
                              >
                                <BiEdit size={14} />
                                Edit
                              </button>
                              <button 
                                ...
                                className="px-3 py-1.5 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs rounded-lg hover:from-red-600 hover:to-red-700 transition-all shadow-md hover:shadow-lg flex items-center gap-1"
                              >
                                <BiTrash size={14} />
                                Delete
                              </button>
                ...
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-gradient-to-r from-blue-500 via-cyan-500 to-blue-500">
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Doctor Name</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Registration</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Specialization</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Contact</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Location</th>
                  ...
                  {doctorPartners.length > 0 ? (
                    doctorPartners.map((partner, idx) => {
                      // Get specialization
                      const specialization = partner.description?.match(/Specialization:\s*([^.]+)/i)?.[1]?.trim() 
                        || partner.contactPerson?.designation 
                        || 'General Physician';
                      
                      // Get qualification
                      const qualification = partner.description?.match(/Qualification:\s*([^.]+)/i)?.[1]?.trim();
                      
                      // Get registration number
                      const registration = partner.documents?.businessLicense 
                        || partner.documents?.gstNumber
                        || partner.adminNotes?.match(/Registration:\s*([^,]+)/)?.[1]?.trim()
                        || null;
                      ...
                      return (
                        <tr key={partner._id} className={`border-b border-gray-200 hover:bg-gradient-to-r hover:from-blue-50 hover:to-green-50 transition-all duration-200 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                          ...
                          <td className="px-4 py-4">
                            ...
                            {qualification && (
                              <div className="text-xs text-gray-500 line-clamp-1">{qualification}</div>
                            )}
                          </td>
                          <td className="px-4 py-4 text-sm">
                            <div className="text-gray-700">{partner.email || partner.contactPerson?.email || 'N/A'}</div>
                          ...
                          <td className="px-4 py-4">
                            {partner.adminNotes?.match(/NGO Offer:\s*([^,]+)/)?.[1] ? (
                              <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">
                                {partner.adminNotes.match(/NGO Offer:\s*([^,]+)/)?.[1].trim()}
                              </span>
                            ) : 'N/A'}
                          </td>
                          <td className="px-4 py-4">
                ...
                <table className="w-full">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-gradient-to-r from-orange-500 via-red-500 to-orange-500">
                      ...
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">FSSAI No.</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Cuisine/Services</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Contact</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Location</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Discount</th>
                      <th className="px-6 py-4 text-left text-sm font-bold text-white uppercase tracking-wide">Actions</th>
                  ...
                  {foodPartners.length > 0 ? (
                    foodPartners.map((partner, idx) => (
                      <tr key={partner._id} className={`border-b border-gray-200 hover:bg-gradient-to-r hover:from-blue-50 hover:to-green-50 transition-all duration-200 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                        <td className="px-4 py-4">
                          <div className="font-medium text-gray-900">{partner.name}</div>
                          <div className="text-xs text-gray-500">{partner.contactPerson?.name || 'N/A'}</div>
                        </td>
                        <td className="px-4 py-4 text-sm text-gray-700">
                          {partner.adminNotes?.match(/FSAI:\s*([^,]+)/)?.[1]?.trim() || partner.documents?.businessLicense || 'N/A'}
                        </td>
                        <td className="px-4 py-4">
                          <div className="text-sm text-gray-700">{partner.description || 'Multi-cuisine'}</div>
                          {partner.description && (
                            <div className="text-xs text-gray-500 line-clamp-1">{partner.description.substring(0, 50)}...</div>
                          )}
                        </td>
                        <td className="px-4 py-4 text-sm">
                          <div className="text-gray-700">{partner.email || partner.contactPerson?.email || 'N/A'}</div>
                        ...
                        <td className="px-4 py-4">
                          {partner.adminNotes?.match(/Special Offer:\s*([^,]+)/)?.[1] ? (
                            <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-medium">
                              {partner.adminNotes.match(/Special Offer:\s*([^,]+)/)?.[1].trim()}
                            </span>
                          ) : 'N/A'}
                        </td>
                        <td className="px-4 py-4">
      ...
      {editingPartner && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className={`sticky top-0 text-white p-6 rounded-t-2xl flex items-center justify-between ${
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b-2 border-gray-200">
                  {editingPartner.businessType === 'hospital' ? '🏥 Hospital Basic Information' : '📋 Partner Information'}
                </h3>
              </div>
                ...
                <>
                  <div className="border-t-2 border-red-200 pt-6 mt-6">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-xl font-bold text-gray-800">Hospital-Specific Details</h3>
                    </div>
                    <p className="text-sm text-gray-600 mb-4">
                      These fields are specific to hospitals. General partner information is shown above.
                    </p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setEditFormData({ ...editFormData, bedCapacity: value });
                        }}
                        min="1"
                        step="1"
                        required
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">Total number of beds available</p>
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2">
                        Specializations
                  ...
                  <div className="mt-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="flex items-center p-3 bg-gray-50 rounded-lg border-2 border-gray-200 hover:border-red-300 transition-colors">
                        ...
                        />
                        <label htmlFor="emergencyServices" className="ml-3 text-sm font-medium text-gray-700 cursor-pointer">
                          🚨 Emergency Services
                        </label>
                      </div>
                      <div className="flex items-center p-3 bg-gray-50 rounded-lg border-2 border-gray-200 hover:border-red-300 transition-colors">
                        <input
                          ...
                          className="w-5 h-5 text-red-600 border-gray-300 rounded focus:ring-red-500 cursor-pointer"
                        />
                        <label htmlFor="ambulanceServices" className="ml-3 text-sm font-medium text-gray-700 cursor-pointer">
                          🚑 Ambulance Services
                        </label>
                      </div>
                      <div className="flex items-center p-3 bg-gray-50 rounded-lg border-2 border-gray-200 hover:border-red-300 transition-colors">
                        <input
                          ...
                          className="w-5 h-5 text-red-600 border-gray-300 rounded focus:ring-red-500 cursor-pointer"
                        />
                        <label htmlFor="icuFacility" className="ml-3 text-sm font-medium text-gray-700 cursor-pointer">
                          🏥 ICU Facility
                        </label>
                      </div>
                      <div className="flex items-center p-3 bg-gray-50 rounded-lg border-2 border-gray-200 hover:border-red-300 transition-colors">
                        <input
                          ...
                          className="w-5 h-5 text-red-600 border-gray-300 rounded focus:ring-red-500 cursor-pointer"
                        />
                        <label htmlFor="operationTheatre" className="ml-3 text-sm font-medium text-gray-700 cursor-pointer">
                          ⚕️ Operation Theatre
                        </label>
                      </div>
                    </div>
